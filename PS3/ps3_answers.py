...
def q4_move(self, motion):
    ...
    if abs(steering) < 0.001:  # Straight line
        res = robot(self.length)
        res.set(self.x + distance * cos(self.orientation),
                self.y + distance * sin(self.orientation),
                self.orientation)
    else:
        turn_radius = self.length / tan(steering)
        beta = distance / turn_radius
        
        # Calculate new position
        cx = self.x - sin(self.orientation) * turn_radius
        cy = self.y + cos(self.orientation) * turn_radius
        
        res = robot(self.length)
        res.set(cx + sin(self.orientation + beta) * turn_radius,
                cy - cos(self.orientation + beta) * turn_radius,
                (self.orientation + beta) % (2.0 * pi))
...
def q5_sense(self, add_noise=1):
    ...
    for landmark in landmarks:
        ...
        # Calculate bearing from robot to landmark
...
def q6_move(self, motion):
    ...
    # Add noise to steering and distance
    steering_noisy = steering + random.gauss(0.0, self.steering_noise)
    distance_noisy = distance + random.gauss(0.0, self.distance_noise)
    
    # Calculate turning radius
    if abs(steering_noisy) < 0.001:  # Straight line
        res = robot(self.length)
        res.set(self.x + distance_noisy * cos(self.orientation),
                self.y + distance_noisy * sin(self.orientation),
                self.orientation)
    else:
        turn_radius = self.length / tan(steering_noisy)
        beta = distance_noisy / turn_radius
