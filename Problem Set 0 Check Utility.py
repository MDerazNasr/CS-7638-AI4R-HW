...
def bool_to_str(x): return str(x)
...
def list_of_float_to_str(lst):
    ...
    if len(lst) > 1:
        for sub_lst in lst:
            if isinstance(sub_lst, float):
                return str(list(map(float_to_str, lst)))  # return string value from a nested float set of lists/tuples
            elif isinstance(sub_lst, list) or isinstance(sub_lst, tuple):
                str_val += str(list(map(float_to_str, sub_lst)))

        return str_val  # return accumulated string value from a list/tuple of floats

    return str(lst)  # return input as a string if unexpected structure encountered
...
def equivalent(arr0, arr1): return arr0 == arr1
...
def check_code_test_case(function_name,
    ...
    if source:
        ...
        for t in range(1, tries + 1):
            try:
                func_name_output = source[function_name](**function_input)
                match_func_output = outputs_match_func(func_name_output, expected_output)

                # need to determine whether PS4 is being executed here or not, if it is then separate the values and policy data
                if function_name == 'q5_stochastic_motion': # this is specific to PS4
                    is_correct, (correct_values, correct_policy) = match_func_output
                    student_vals, student_policy = func_name_output
                    func_name_output = (student_vals, student_policy, correct_values, correct_policy)

                    is_values_correct = all([_v for _r in correct_values for _v in _r])
                    is_policy_correct = all([_v for _r in correct_policy for _v in _r])
                    if False in (is_values_correct, is_policy_correct):
                        detailed_results += "Incorrect results are indicated by (parentheses).\n\n"
                else:
                    is_correct = match_func_output

            except Exception:
                func_name_output = None
                is_correct = False
            ...
            if is_correct:
                ...
                try:
                    detailed_results += output_to_str_func(func_name_output) + "\n"
                except Exception:
                    stack_trace = traceback.format_exc()
                    print(stack_trace)
                    detailed_results += str(func_name_output) + "\n"
